(function () {
	...
	var handleScoreFlashTrans = function($elem, amount){
		if (amount >= 100 && amount < 500){
			$elem.css({"font-size": "60px", "color": "#FF9933"});
		} else if (amount >= 500 && amount < 1000){
			$elem.css({"font-size": "80px", "color": "#FF6600"});
		} else if (amount >= 1000){
			$elem.css({"font-size": "100px", "color": "#FF0000"});
		} else {
			$elem.css({"font-size": "40px", "color": "#FFFF00"});
		};
	...
	Utils.updateLives = function() {
		window.LIVES -= 1;
		if (window.options.runTransitions) {
			$("#lives").css({"background-color": "orange"});
