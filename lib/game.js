(function () {
  ...
  Game.prototype.checkForFleaAndSpiderCreation = function () {
  	if (!this.flea && window.MASTER_TIMER > 0 && window.MASTER_TIMER % 120 === 0) {
  		var num = Math.random();
  		if (num > 0.5) {
  			this.flea = new Centipede.Flea(this);
  		}
  	}
  	// if (!this.spider && window.MASTER_TIMER > 0 && window.MASTER_TIMER % 300 === 0) {
  		// var num = Math.random();
  		// if (num > 0.5) {
