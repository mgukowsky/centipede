(function () {
  ...
  var Game = Centipede.Game = function () {
   ...
   this.mushrooms = Centipede.Mushroom.seedMushrooms();
   this.bullet = null;
  };
  ...
  Game.prototype.handleMushroomDestroy = function() {
  	var shouldDestroyBullet = Centipede.collisionChecks.bulletWithMushroom(this.bullet, this.mushrooms);
    if (shouldDestroyBullet) {
 			delete Centipede.Debugger.hitboxes.bullet; //Explicitly garbage collect bullet hitbox;
			delete this.bullet;
    }
  ...
  Game.prototype.draw = function (ctx, images) {
    ...
    if (this.bullet){
		...
		for (mushroom in this.mushrooms) {
