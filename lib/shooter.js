(function () {
	...
	Shooter.prototype.move = function(options) {
		if (options[37] && this.x > this.MIN_X && !this.willCollideWithMushroom([this.x - this.vel, this.y])) {
			this.x -= this.vel;
			this.hitbox.origin[0] -= this.vel;
		};
		//Neutralize if L & R are pressed
		if (options[39] && this.x < this.MAX_X && !this.willCollideWithMushroom([this.x + this.vel, this.y])) {
			this.x += this.vel;
			this.hitbox.origin[0] += this.vel;
		};
		if (options[38] && this.y > this.MIN_Y && !this.willCollideWithMushroom([this.x, this.y - this.vel])) {
			this.y -= this.vel;
			this.hitbox.origin[1] -= this.vel;
		};
		//Neutralize if U & D are pressed
		if (options[40] && this.y < this.MAX_Y && !this.willCollideWithMushroom([this.x, this.y + this.vel])) {
			this.y += this.vel;
			this.hitbox.origin[1] += this.vel;
		};
	};
	...
	Shooter.prototype.willCollideWithMushroom = function (pos) {
		...
		var willCollide = Centipede.collisionChecks.shooterWithMushroom(hitbox, this.context.shooterCollisionMushrooms);
