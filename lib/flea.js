(function () {
	...
	var Flea = Centipede.Flea = function (context, options) {
		...
		this.y = 0;
		this.vel = 6  / window.frameRateMod;	// Bullets travel in a straight line, so only 1D vector is needed.
	...
	Flea.prototype.layMushroom = function() {
		if (this.canLayMush) {
			...
			if (num <= 50 && this.y <= 560) {
				var mush =  Centipede.Mushroom.generateMushroom(window.mushroomIndex, this.x, this.y, this.context);
